...
def _normalize_meshtatic_format(topic, payload):
    """Normalize Meshtatic message.
    ...
    if "from" not in payload:
        LOG.warning("missing 'from' in meshtatic payload")
        return topic, payload

    if "json" not in topic:
        LOG.warning("Meshtatic protobuf not supported")
        return topic, payload
...
def _parse_message(raw_topic, raw_payload):
    ...
    elif raw_topic.startswith(settings.MESHTATIC_TOPIC_PREFIX):
        topic, payload = _normalize_meshtatic_format(raw_topic, payload)
